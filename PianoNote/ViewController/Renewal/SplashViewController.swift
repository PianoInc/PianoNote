...
class SplashViewController: DRViewController {
    ...
    /*let originY = safeInset.top + naviHeight
     ...
     button4.frame = CGRect(x: width * 3, y: originY, width: width, height: height)
     button4.backgroundColor = .yellow
     button4.setTitle("Info", for: .normal)
     button4.setTitleColor(.black, for: .normal)
     button4.titleLabel?.font = UIFont.systemFont(ofSize: 18, weight: .bold)
     view.addSubview(button4)
     _ = button4.rx.tap.subscribe { _ in
     self.present(viewCtrl: UIStoryboard.view(type: InfoViewController.self))
     }
     }
